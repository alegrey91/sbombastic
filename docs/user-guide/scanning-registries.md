...
Another way to reduce the scanning load is to apply filters to the image tags. To use this filter, you have to use the `matchConditions` field, provided under `repositories`.

This field allows the user to write their own [CEL](https://kubernetes.io/docs/reference/using-api/cel/) expressions to filter for image tags. Let's see an example:
